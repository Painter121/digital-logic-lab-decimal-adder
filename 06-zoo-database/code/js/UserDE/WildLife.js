...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("http://localhost:3000/WildLife/getTerrestrial")
        .then((response) => response.json())
        .then((data) => {
            // Generate cards for each animal
            data.forEach((animal) => {
                ...
                card.innerHTML = `
                    <img src="${animal.Image || 'default-image.jpg'}" alt="${animal.Nickname}">
                    <div class="info">
                        <h3>${animal.Nickname}</h3>
                        <p>${animal.Species} (${animal.Gender})</p>
                        <p>Zone: ${animal.ZoneName}</p>
                    </div>
                `;
...
function smoothParallax() {
    currentScrollY += (lastScrollY - currentScrollY) * 0.1; // เพิ่มความนุ่มนวล
    const scrollSpeed = 0.5;
